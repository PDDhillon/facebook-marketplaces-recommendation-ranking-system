...
Facebook Marketplace is a market-leading platform for buying and selling products online. The hallmark of this platform is the speed and efficiency at which it can recommend products to a user, based on their previously viewed items. Behind the scenes, Facebook is able to process image data alone in order to categorise and rank images. This ranking system provides a mathematical representation of the similarity of two images and stores it in a Facebook AI Similarity Search (FAISS) index.
...
## 1. Usage
To use the final solution, clone the reppository and run the ```my-app``` folder for the React application and the ```app``` folder for the API.
...
- Javascript/CSS/HTML
  - React
  - ChakraUI
- Cloud Services/Containerised Development
  - Docker
  - AWS
    - Amplify
    - EC2
...
Using the feature extraction model that was created previously, a dictionary was created to represent all of the images in our dataset. The key was the image filename, and the value was the image embedding produced by the feature extraction model. This was added to a FAISS index and saved to a pkl file ```index.pkl```. This meant that the index could be recreated again with the exact same values.

### 3.4 Configure and deploy the model serving API
Once all the components had been created, it was time to link them together to be consumed. The two endpoints, created using FastAPI and uvicorn were 1) A GET endpoint to retrieve an image embedding from a single image and 2) a POST to get the index of similar images from the one posted. These endpoints were containerised and stored inside a docker image. 

Docker images were the perfect solution to deploy our application to our AWS EC2 instance. Ultimately, if this was a production application, we could scale the containers based on usage to provide load balancing. This would prevent users from a downturn in performance.
...
However, given the constraints and issues mentioned, the model is still able to provide successful predictions and return similar images. The wide variety of technologies and libraries used made this an exceptional learning project.
